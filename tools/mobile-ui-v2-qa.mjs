...
const screenshotDir = resolve("screenshots/mobile-v2");
const auditPath = resolve("mobile-v2-audit.md");
const port = 4179;
...
const pages = [
  ...
  ["search-hardware", "/products.html?q=hardware"],
  ["image-search-modal", "/index.html", { imageSearch: true }],
  ["quote-contact-prefill", "/contact.html?sku=ABS-HDW-001&product=Hardware%20Product%2001"],
...
async function waitForStablePage(page) {
  ...
  await page.evaluate(async () => {
    ...
    const timeout = new Promise((resolveWait) => window.setTimeout(resolveWait, 900));
...
async function collectMetrics(page) {
  return page.evaluate(() => {
    ...
    return {
      ...
      brandClipped: brandName ? (
        brandStyle.textOverflow === "ellipsis" ||
        (brandStyle.overflow !== "visible" && brandName.scrollWidth > brandName.clientWidth + 1)
      ) : false,
...
try {
  ...
  for (let index = 0; index < pages.length; index += 1) {
    ...
    console.log(`screenshot ${index + 1}/${pages.length}: ${slug}`);
...
const pageSections = screenshotRows.map((row) => {
  const pageIssues = qaRows.filter((qa) => qa.path === new URL(row.url).pathname + new URL(row.url).search).filter((qa) => (
    qa.bodyOverflow > 1 || qa.clippedCards > 0 || qa.brandClipped || qa.cameraButtons < 1 || qa.menuButtons < 1 || qa.whatsappOverlap
  ));
  return [
    `### ${pageDisplayName(row.slug)}`,
    `- URL: ${row.url}`,
    `- Screenshot: screenshots/mobile-v2/${row.file}`,
    ...
    `- Remaining Issues: ${pageIssues.length ? pageIssues.map((issue) => `${issue.width}px overflow=${issue.bodyOverflow}, clippedCards=${issue.clippedCards}, brandClipped=${issue.brandClipped}, camera=${issue.cameraButtons}, menu=${issue.menuButtons}, whatsappOverlap=${issue.whatsappOverlap}`).join("; ") : "None detected in automated viewport checks."}`,
...
const audit = `# Aurora Mobile Design System V2 Audit
...
${hardIssues.length ? hardIssues.map((row) => `- ${row.width}px ${row.path}: overflow=${row.bodyOverflow}, clippedCards=${row.clippedCards}, brandClipped=${row.brandClipped}, camera=${row.cameraButtons}, menu=${row.menuButtons}, whatsappOverlap=${row.whatsappOverlap}`).join("\n") : "- None detected by automated checks."}
